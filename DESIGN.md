...
- Public marketing controls render as compact 36px pills in the header and larger 40??8px CTAs deeper in the page.
...
Karrot speaks like a trustworthy neighbor who just moved in next door: warm, plain-spoken, low-friction, and allergic to anything that sounds like corporate marketing. The voice assumes two strangers are about to hand each other a used crib across a parking lot ??it protects that trust with earnestness, removes barriers with `遺???놁씠` (without burden) framing, and stays in everyday Korean sentence endings (`-?댁슂`, `-?덉슂`) rather than the formal `-?귣땲??. English surfaces (Karrot in US/Canada/UK/JP) mirror this in plain, contraction-friendly English ??*"Buy and sell for free with locals"*, not *"Discover premium local marketplace experiences"*.

| Context | Tone |
|---|---|
| CTAs | Short verb-first Korean (`?먮ℓ?섍린`, `梨꾪똿?섍린`, `嫄곕옒 ?꾨즺`) / plain imperative English (`Sell`, `Chat`, `Apply`) |
| Empty states | One warm line explaining *why it's empty* + one low-pressure suggestion. Never `?곗씠?곌? ?놁뒿?덈떎`. |
| Error messages | Specific, blameless, actionable. Prefer `?ㅼ떆 ?쒕룄??二쇱꽭?? over `?ㅻ쪟媛 諛쒖깮?덉뒿?덈떎`. |
| Success toasts | Past-tense single sentence (`嫄곕옒媛 ?꾨즺?섏뿀?댁슂`). Quiet, not celebratory. |
...
What Karrot refuses: the anonymity of nationwide marketplaces (eBay, Coupang), the impersonal aesthetics of enterprise commerce (data-heavy dashboards, filter-rich search UIs), and the gamified engagement loops of consumer social (streaks, badges, algorithmic feeds). The brand's mission, stated on its own corporate page, is *"濡쒖뺄??紐⑤뱺 寃껋쓣 ?곌껐???숇꽕???⑥? 媛移섎? 湲곗닠濡?源⑥슦??* ??connecting everything local, awakening hidden neighborhood value through technology ([medium.com/daangn](https://medium.com/daangn)). Orange is the accent because the brand is supposed to feel like one warm thing in an otherwise neutral room.
...
3. **Proximity is surfaced, always.** Every listing, chat, and search result shows a neighborhood name. Distance is not a filter you have to remember to toggle ??it's a default. *UI implication:* every card, row, or summary that represents user content must show the neighborhood (`?? / `neighborhood`) as visible metadata, not hidden in a detail screen.
...
8. **Content-dense, chrome-light.** Users scan many listings in one session ??a listing card is closer to an SMS than to a Pinterest tile. *UI implication:* target 3?? listings visible per mobile viewport; chrome (borders, shadows, decorative space) must not push that below 3.
...
| State | Treatment |
|---|---|
| **Empty (no listings nearby)** | Warm one-line explanation (`?꾩쭅 ?곕━ ?숇꽕???щ씪??臾쇨굔???놁뼱??) + one secondary CTA in neutral-weak (`???숇꽕 諛붽씀湲? / change neighborhood). Never an illustration. Never `?곗씠?곌? ?놁뒿?덈떎`. |
| **Empty (filter cleared)** | Single line of `gray-700` caption (`議곌굔??留욌뒗 臾쇨굔???놁뼱??). No button ??user resets the filter themselves. |
| **Empty (new user, first paint)** | Single welcome sentence naming the user's detected neighborhood, plus a primary CTA `?섎윭蹂닿린` (browse) in Karrot Orange. No onboarding carousel. |
...
| **Error (inline field)** | Input border becomes `#fa342c` (red-700) 2px, helper text below in red-700 13px. One actionable sentence (`?숇꽕瑜??ㅼ떆 ?좏깮??二쇱꽭??). |
...
| **Success (transaction complete)** | Dedicated confirmation screen ??not a toast. `#079171` (positive green) check icon top-center, one-line past-tense sentence (`嫄곕옒媛 ?꾨즺?섏뿀?댁슂`), and a single primary button `留ㅻ꼫 ?됯? ?④린湲? (leave manner rating). |
...
3. **Neighborhood switch.** When the user changes their ??(neighborhood), the listings feed cross-fades over `motion-slow` rather than sliding ??sliding would imply geographic direction, which is misleading (Korean neighborhoods aren't ordered on an axis).
...
<!--
OmD v0.1 Sources ??Philosophy Layer (sections 10??5)
...
- https://about.daangn.com ??corporate site confirms mission tagline
  "?숇꽕瑜??щ뒗 臾? ?밴렐", full mission statement "濡쒖뺄??紐⑤뱺 寃껋쓣 ?곌껐??
  ?숇꽕???⑥? 媛移섎? 源⑥썙??, Jan-2025 metrics (40M+ registered users,
  20M+ MAU, 1,400+ regions, 227B KRW funding).
- https://medium.com/daangn ??engineering publication confirms
  "濡쒖뺄??紐⑤뱺 寃껋쓣 ?곌껐???숇꽕???⑥? 媛移섎? 湲곗닠濡?源⑥슦?? as the tech-focused
...
- https://www.kedglobal.com/korean-startups/newsView/ked202407040005 ??  confirms initial 6 km transaction radius, later 10 km (KR/JP) and up to
  50 km (US/CA), Canada expansion (2024).

Base DESIGN.md (sections 1??) is the source for token-level claims including
...
- The 40M+ registered / 20M+ MAU / 1,400+ regions / 227B KRW figures in 짠11 are
...
- Korean "?숇꽕" (neighborhood / dong) is a real administrative unit; the
  characterization of nationwide Korean e-commerce incumbents is general
  industry knowledge, not a sourced Karrot statement.

Personas (짠13) are fictional archetypes informed by publicly described Karrot
user segments (KR urban young adult, KR secondary-city student, NA expat,
KR retiree ?숇꽕?앺솢 user). Any resemblance to specific individuals is unintended.
...
- "Orange is the accent because the brand is supposed to feel like one warm
  thing in an otherwise neutral room" (짠11 closing) ??editorial reading of the
  design, not a sourced brand statement.
- The characterization of the Karrot orange as "fresh carrot, not corporate
  orange, not alarm orange" (짠11) ??editorial framing based on observed usage.
- The spring-forbidden stance (짠15) ??derived from the overall brand posture
  (trust between strangers, calm neutrality) as expressed in base 짠6 Shadow
  Philosophy and 짠7 Do's/Don'ts; not a documented Seed Design rule.
-->
